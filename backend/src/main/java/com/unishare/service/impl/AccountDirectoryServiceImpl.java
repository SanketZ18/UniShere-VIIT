...
public class AccountDirectoryServiceImpl implements AccountDirectoryService {
    ...
    public UserProfileResponse buildProfile(UserAccount account) {
        if (account.getRole() == Role.STUDENT) {
            ...
            return PortalMapper.toUserProfile(account, student.getFullName(), student.getDepartment(), student.getMobile(), student.getGender(), student.getBirthDate());
        }
        Staff staff = staffRepository.findById(account.getUserId())
                .orElseThrow(() -> new ResourceNotFoundException("Staff profile not found"));
        return PortalMapper.toUserProfile(account, staff.getFullName(), staff.getDepartment(), staff.getMobile(), staff.getGender(), staff.getBirthDate());
    }
